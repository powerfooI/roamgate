...
test.skipIf(!chrome)(
  "themed select anchors to its trigger and stacks above modal backdrops",
  async () => {
    const dir = await mkdtemp(join(tmpdir(), "themed-select-test-"));
    ...
    const server = Bun.serve({
      ...
      async fetch(request) {
        ...
        if (path === "/") {
          return new Response(
            '<head><link rel="stylesheet" href="/ThemedSelect.browser.css"></head><body><script src="/ThemedSelect.browser.js"></script></body>',
    ...
    try {
      const build = await Bun.build({
        entrypoints: [join(import.meta.dir, "ThemedSelect.browser.tsx")],
        outdir: dir,
        target: "browser",
      });
