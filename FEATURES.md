...
- Multiple browsers receive pushed events. Pause/resume yours, see client counts,
  or pause others. **Task notifications** use Service Worker notifications when
  supported, with a page-notification fallback for older browsers. Clicking a
  notification focuses or opens Roamgate and returns to the relevant pane when
  its connection runtime is still current. Notification failures disable the
  setting and show an error.
  On iPhone/iPad, use the HTTPS Home Screen app on iOS/iPadOS 16.4 or later and
  grant notification permission. Task events are observed only while the page
  runs; a suspended or closed app does not receive new task notifications.
